...
RSpec.describe OtpRateLimiter do
  ...
  subject(:otp_rate_limiter) do
    OtpRateLimiter.new(phone: current_user.phone, user: current_user, phone_confirmed: false)
  end
  subject(:otp_rate_limiter_confirmed) do
    OtpRateLimiter.new(phone: current_user.phone, user: current_user, phone_confirmed: true)
  end
  ...
  describe '#increment' do
    it 'updates otp_last_sent_at' do
      tracker = OtpRequestsTracker.find_or_create_with_phone_and_confirmed(current_user.phone,
                                                                           false)
